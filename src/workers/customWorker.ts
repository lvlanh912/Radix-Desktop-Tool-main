  ...
  if (action.executor === "ManifestExecute") {
    switch (action.method) {
      case "execute":
        ...
        break;
      case "preview":
        ManifestExecute.preview(msg.data);
        break;
    }
  }
};
...
const ManifestExecute = {
  ...
  preview: async (data: Data) => {
    const manifestStr = data.args[0] as string;
    const currentEpoch = data.args[1] as number;
    const feePayerPrivateKey = data.args[2] as string;
    const privateKeyList = JSON.parse(data.args[3]) as string[];
    const networkId = data.args[4] as number;
    const order = data.args[5] as number;

    const pkList: PublicKey[] = [];
    ...
    executor
      .executePreview(manifestStr, pkList, currentEpoch)
      .then((result) => {
        if (result.errorMessage) {
          console.error(result.errorMessage);
          result.fee = "error";
